### Calculate transitions using TF_ALL data (if any TFBS for a TF has a motif, count as 1 else 0)
###
...
matrix_data <- read.table('../z_data/NEW_CRE_hATs_rewired_trinary_matrix_for_modelling_ANY_TFBS_BINARY_Oct2025.txt', header = T)

#### Writing a loop to run simmap for the re-wired TFBSs RUN ON WENDY ####
# 
...
simmap_list <- readRDS('../z_data/Z_simmaps_11rewired_TFs_BINARY_for_rewiring_x100_reps.rds') # Find on Zenodo

# Pick a specific simmap, e.g., the 3rd TFBS, 1st simulation out of 100
# sim_num <- 51
# simmap_obj <- simmap_list[[which(names(simmap_list) == 'Creb3l2_ANY')]][[sim_num]]
# trait_name <- names(simmap_list)[which(names(simmap_list) == 'Creb3l2_ANY')]
...
for (trait in names(simmap_list)) {
  ...
  # Initialize tibble for this trait
  res_trait <- tibble(
    trait = trait,
    sim   = seq_len(n_sims),
...
trait_summary <- trait_summary %>%
  ...
  mutate(trait = factor(trait, levels = trait))  # keep ordering in plot



load("p1_plot.RData") # From Fig41B
mean_upstream_gains_sum_by_TF <- p1$data %>% 
  mutate(TF = str_replace(trait, "\\.\\d+$", "")) %>% 
  group_by(TF) %>% 
  summarize(mean_upstream_gains_sum = sum(mean_upstream_gains)) %>% 
  mutate(TF_any = paste0(TF, '_ANY'))

trait_summary <- trait_summary %>% 
  left_join(mean_upstream_gains_sum_by_TF %>% select(mean_upstream_gains_sum, TF_any), 
            by = c('trait'='TF_any'))

p2 <- ggplot(trait_summary,
             aes(x = mean_upstream_gains_sum,
                 y = reorder(trait, mean_upstream_gains_sum))) +
  geom_point(aes(size = mean_CREs_affected),
             color = "steelblue", alpha = 0.7) +
  theme_minimal() +
  ylab("TFBS Trait") +
  xlab("Mean Upstream Gains") +
  ggtitle("Balloon plot of mean upstream gains vs. mean CREs affected")
